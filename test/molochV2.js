    ...
    if (didPass) {
      ...
      if (initialApplicantShares > 0) {
        ...
        assert.equal(
          ...
          'member shares incorrect'
        )
      } else {
        const newMemberData = await moloch.members(proposal.applicant)
        assert.equal(newMemberData.delegateKey, proposal.applicant, 'member delegateKey does not match')
        assert.equal(newMemberData.shares, proposal.sharesRequested, 'member shares does not match')
        assert.equal(newMemberData.exists, true, 'member exists does not match')
        assert.equal(newMemberData.highestIndexYesVote, 0, 'member highestIndexYesVote does not match')

        const newMemberAddressByDelegateKey = await moloch.memberAddressByDelegateKey(proposal.applicant)
        assert.equal(newMemberAddressByDelegateKey, proposal.applicant, 'member delegate address does not match')
      }
    }
  }
})
